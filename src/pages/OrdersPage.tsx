  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {orders.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-xl shadow-lg transform transition-all duration-300 hover:scale-[1.01]">
            <div className="relative inline-block">
              <Package className="mx-auto h-20 w-20 text-blue-400" />
              <div className="absolute inset-0 bg-blue-400 opacity-20 blur-xl rounded-full" />
            </div>
            <h3 className="text-2xl font-semibold text-gray-900 mt-6 mb-3">{t.noOrders}</h3>
            <p className="text-gray-600 mb-8 max-w-md mx-auto">{t.noOrdersDesc}</p>
            <Button 
              onClick={() => navigate('/shop')}
              className="bg-blue-500 hover:bg-blue-600 text-white px-8 py-3 rounded-lg transform transition-transform hover:scale-105"
            >
              {t.startShopping}
            </Button>
          </div>
          ...
          <div className="space-y-6">
            {orders.map((order) => (
              <div key={order.id} className="bg-white rounded-xl shadow-lg p-8 transform transition-all duration-300 hover:scale-[1.02]">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center space-x-4">
                    <div className="relative">
                      {getStatusIcon(order.status)}
                      <div className="absolute inset-0 opacity-20 blur-sm rounded-full bg-current" />
                    </div>
                    <div>
                  ...
                  <div className="text-right">
                    <div className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-green-500 to-green-600 mb-2">
                ...
                <div className="flex items-center justify-between border-t border-gray-100 pt-4 mt-4">
                  <div className="flex items-center space-x-6 text-sm text-gray-600">
                    <div className="flex items-center space-x-2">
                      ...
                      <span>{t.items}: {order.order_items?.length || 0}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Clock className="w-4 h-4 text-gray-400" />
                      <span>{t.paymentMethod}: {getPaymentMethodText(order.payment_method)}</span>
                    </div>
                  </div>
